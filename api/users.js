...
router.post('/open-case', async (req, res) => {
    ...
    console.log('Open case request:', req.body); // Логирование запроса
    ...
    try {
        ...
        const totalCost = caseInfo.price * count;
        ...
        if (!is_demo) {
            ...
            await client.query(
                ...
                [user_id, -totalCost, `Открытие кейса ${caseInfo.name} x${count}`]
            );
        }

        // 3. Получаем предметы кейса
        ...
        for (let i = 0; i < count; i++) {
            const totalChance = items.reduce((sum, item) => {
                ...
                return sum + (isNaN(chance) ? 0 : chance);
            }, 0);
        ...
        console.log('Case opened successfully', { items: wonItems });
        return res.json({
            success: true,
            items: wonItems,
            totalCost: is_demo ? 0 : totalCost
        });
